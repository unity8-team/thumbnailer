...
class WorkerThread : public QThread
{
    Q_OBJECT
public:
    WorkerThread(QString download_id, QObject *parent=nullptr) : QThread(parent), download_id_(download_id)
    {
    }
...
private:
    void run() Q_DECL_OVERRIDE
    {
        ...
        QString url_to_check = QString("/musicproxy/v1/album-art?artist=test_threads&album=%1&size=350&key=0f450aa882a6125ebcbfb3d7f7aa25bc").arg(download_id_);
...
TestDownloader::TestDownloader() : fake_downloader_server_(posix::ChildProcess::invalid())
{
}
...
void TestDownloader::test_ok_album()
{
    ...
    QVERIFY(url.endsWith("/musicproxy/v1/album-art?artist=sia&album=fear&size=350&key=0f450aa882a6125ebcbfb3d7f7aa25bc") == true);
    ...
    // expected.
    QList<QVariant> arguments = spy.takeFirst();
    QCOMPARE(arguments.at(0).toString().endsWith("/musicproxy/v1/album-art?artist=sia&album=fear&size=350&key=0f450aa882a6125ebcbfb3d7f7aa25bc"), true);
    // Finally check the content of the file downloaded
...
void TestDownloader::test_ok_artist()
{
    ...
    QVERIFY(url.endsWith("/musicproxy/v1/artist-art?artist=sia&album=fear&size=300&key=0f450aa882a6125ebcbfb3d7f7aa25bc") == true);
    ...
    // expected.
    QList<QVariant> arguments = spy.takeFirst();
    QCOMPARE(arguments.at(0).toString().endsWith("/musicproxy/v1/artist-art?artist=sia&album=fear&size=300&key=0f450aa882a6125ebcbfb3d7f7aa25bc"), true);
    // Finally check the content of the file downloaded
    QCOMPARE(arguments.at(1).toString(), QString("SIA_FEAR_TEST_STRING_IMAGE"));
}
...
void TestDownloader::test_not_found()
{
    ...
    QVERIFY(url.endsWith("/musicproxy/v1/album-art?artist=test&album=test&size=350&key=0f450aa882a6125ebcbfb3d7f7aa25bc") == true);
    ...
    // expected.
    QList<QVariant> arguments = spy.takeFirst();
    QCOMPARE(arguments.at(0).toString().endsWith("/musicproxy/v1/album-art?artist=test&album=test&size=350&key=0f450aa882a6125ebcbfb3d7f7aa25bc"), true);
    QCOMPARE(arguments.at(1).toInt(), static_cast<int>(QNetworkReply::InternalServerError));
    QCOMPARE(arguments.at(2).toString().endsWith("/musicproxy/v1/album-art?artist=test&album=test&size=350&key=0f450aa882a6125ebcbfb3d7f7aa25bc - server replied: Internal Server Error"), true);
}
...
void TestDownloader::test_not_found_url()
{
    ...
    QCOMPARE(arguments.at(2).toString().endsWith("images_not_found/sia_fear_not_found.png - server replied: Not Found"), true);
}
